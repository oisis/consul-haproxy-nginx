...
if [[ `hostname` == "lb" ]]; then
  ...
  /usr/bin/consul-template -config /root/haproxy/consul-template-haproxy.hcl 2>&1 >/dev/null &
...
if [[ `hostname` == "consul-server" ]]; then
  /usr/bin/curl -s -X PUT -d '4096' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/maxconn
  /usr/bin/curl -s -X PUT -d '5s' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/timeout-connect
  /usr/bin/curl -s -X PUT -d '50s' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/timeout-client
  /usr/bin/curl -s -X PUT -d '50s' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/timeout-server
  /usr/bin/curl -s -X PUT -d 'enable' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/stats
  /usr/bin/curl -s -X PUT -d '5s' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/refresh
  /usr/bin/curl -s -X PUT -d '/' http://172.20.20.31:8500/v1/kv/prod/portal/haproxy/uri
fi
